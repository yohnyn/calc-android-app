...
- 首页结果卡支持保存方案和查看详情；保存按参数去重。
- 结果详情突出净盈亏、保证金收益率、可选止盈/止损和仓位安全边界；交易参数默认展开，公式默认折叠。
- 方案对比支持不完整首页状态下独立导入方案，并允许选择任意完整方案作为基准。
- 方案库保存完整输入，支持重命名、备注、复制、加入对比、带入补仓助手、单个删除和全部删除。
...
- 最新变更跨越多个 Compose 调用签名，优先确认是否存在编译错误。
- 小额币数量显示、补仓数量、方案删除联动和补仓页面自动定位需要真机验证。

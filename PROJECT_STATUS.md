...
- 计算历史本地保存
- 补仓展开状态、主题模式和币本位默认模式持久化；首页模块顺序固定，不再支持排序或显隐
...
- 2026-06-08 优化 2.0 Final Freeze 收敛：产品定位更新为“仓位助手”；首页顺序固定并删除模块排序/显隐设置、状态和偏好仓储逻辑；`高级设置` 改为 `费率设置`；U 本位全仓账户总资产改为仓位参数里的 `全仓模式` 复选项；`止盈止损` 改为复选框控制并直接输入止盈价/止损价；计算结果新增止盈收益、止损亏损和盈亏比；结果与复制文案删除风险等级/风险高低评价；收益方案对比删除排名、最终排序和最优/最佳方案口径；杠杆选择器改为常用倍数 + 更多展开。该轮仅做非编译静态检查，未运行 Gradle、编译或测试。
...
- 最近一次结果展示 UI 和收益方案对比 UI 拆分尚未由用户本地编译或真机验收；重点确认主结果弹窗、币本位结果弹窗、首页简略结果卡、净盈亏/ROI/手续费/强平价展示，以及收益方案对比选择/添加/编辑/删除/弹窗结果/历史保存
- 最近一次主屏输入结构与止盈止损 UI 调整尚未由用户本地编译或真机验收；重点确认平仓价移入仓位参数、止盈止损默认收起/展开、账户总资金位置、止盈止损目标反推详情弹窗，以及添加/编辑对比方案弹窗新分组布局
- 最新优化 2.0 Final Freeze 收敛尚未由用户本地编译或真机验收；重点确认固定首页顺序、设置页删除模块管理、全仓模式账户总资产复选框、止盈止损复选框、止盈收益/止损亏损/盈亏比、收益对比无排名/无最优方案、杠杆更多展开和复制文案无风险等级
...
- `CalculationInput` 已新增直接 `止盈价` / `止损价` 字段，并已贯通主结果展示；旧目标收益/ROI、目标亏损/ROI 字段仍保留兼容。新增字段和计算链路尚未本地编译验证
- 首次复制选择摘要版/详细版并记住选择、设置页复制默认格式、方案库管理、更新日志、历史收藏到方案库尚未完整实现
...
- `CalculatorScreen.kt` 仍约 706 行，后续维护风险已明显下降；后续拆分应继续遵循“每次只拆一个功能模块、保持 UI 与计算行为不变”的节奏
...
- 已按优化 2.0 Final Freeze 完成本轮源码收敛：固定首页顺序并删除模块排序/显隐；改造全仓模式、止盈止损、杠杆更多、设置页分组、收益对比客观展示、结果页和复制文案禁用口径；新增直接止盈价/止损价及止盈收益、止损亏损、盈亏比计算；已更新 `APP_DEVELOPMENT_SPEC.md`、`CURRENT_TASK.md`、`AI_HANDOFF.md`

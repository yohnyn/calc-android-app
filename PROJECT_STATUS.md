...
- 对比方案 UI 已可选择币本位和币本位计算方式，但需要用户本地确认 ViewModel/ComparisonCalculator 对该字段的实际计算支持是否已完全贯通；如未贯通，可能只是展示/保存层面的增强
...
- 重点验证收益方案对比新增/编辑流程：U 本位/币本位切换、币本位计算方式选择、方案列表展示、对比结果详情、历史快照字段是否符合预期
